...
// ReceiveError tries to fetch the most recent error
// string values (as opposed to the 9P2000.L errno)
...
func ReceiveError(attachRoot p9.File, srvErr error) error {
	...
	fail := func(err error) error {
		return fmt.Errorf(
			"could not retrieve error string from ErrFS because: %w"+
				"\nraw client error: %w",
			err, srvErr,
		)
	}
	...
	if len(errorBytes) > 0 {
		return fmt.Errorf(
			"server-side error: %s",
			generic.ConvertBytes(errorBytes),
		)
	}
	return srvErr
}
