...
func TestParameters(t *testing.T) {
	var (
		...
		request, requestError = cmds.NewRequest(ctx, nil,
			cmds.OptMap{
				fscmds.ServiceMaddrs().CommandLine():    expectedMaddrStrings,
				fscmds.AutoExitInterval().CommandLine(): expectedStopInterval.String(),
			},
			nil, nil, root)
	)
	defer cancel()
	...
	{ // Round 1 - full set
		...
		if settings.AutoExitInterval != expectedStopInterval {
			t.Fatal("TODO something about time being wrong")
		}
	}

	{ // Round 2 - partial set
		delete(request.Options, fscmds.AutoExitInterval().CommandLine())
		var (
			settings        = new(fscmds.Settings)
			unsetArgs, errs = parameters.ParseSettings(ctx, settings,
				parameters.SettingsFromCmds(request),
			...
			expectedUnsetParam = fscmds.AutoExitInterval()
		)
		if err != nil {
			t.Fatal(err)
		}
